...
We now have $M_i$ (yield of species $i$ in monoculture) and 
$Y_{O,i}$ (observed yield of species $i$ in mixture):
...
The total observed yield of the mixture $Y_O$ is 
the summed observed yield of all species in mixture $∑_i Y_{O,i}$ :
...
The expected relative yield ($RY_{E,i}$) for each species in the 4-species mix 
is 0.25 and 0.0625 for each species in the 16-species mix (i.e., 1/4 and 1/16).

To calculate the expected yield of species $i$ in the mixture ($Y_{E,i}$)
we can do $RY_{E,i}M_i$ 
(the expected relative yield multiplied by the yield of the species in monoculture):
...
and the total expected yield of the mixture ($Y_E$) 
is those values summed across species $∑_iY_{E,i}$ :
...
So now we can calculate $𝚫Y$, the deviation from total expected relative yield 
in the mixture as: $Y_{O} - Y_{E}$
...
The complementarity effect is $N\overline{𝚫RY} \overline{M}$,
the number of species in the mixture ($N$) multiplied by the 
mean $𝚫RY_i$ and mean $M_i$ across species.
